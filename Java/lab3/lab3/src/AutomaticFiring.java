...
public class AutomaticFiring extends GardenDevice{
    ...
    public void changeWorkIntensivity(short intensivity){
        if (!powered_on) { System.out.println("Прежде чем поменять длительность поливки включите автополив");}
        else if (intensivity == 1 || intensivity == 2 || intensivity == 3 || intensivity == 4){
            switch (intensivity){
                case 1:
                    watering_duration = 60;
                    System.out.println("Длительность полива состовляет: " + watering_duration + " минут");
                    break;
                case 2:
    ...
    public void deviceOperation(){
        ...
        if (wear_level >= 90){
            System.out.println("Система полива слишком изношена. Нужна нужен ремонт");
            return;
        }
        ...
        water_tank_level = clamp(water_tank_level - water_used, 0, 100);
        soil_moisture = clamp(soil_moisture + moisture_increase, 0, 100);
    ...
    public void switchZone(int zone){
        active_zone = clamp(zone, 1, 10);
        System.out.println("Полив переключен на зону " + active_zone);
    }

    @Override
    public void randomizeSpecificProperties(){
        water_tank_level = clamp(water_tank_level + randomValue(-20, 10), 0, 100);
        watering_duration = clamp(watering_duration + randomValue(-10, 10), 1, 120);
        soil_moisture = clamp(soil_moisture + randomValue(-15, 15), 0, 100);
        active_zone = clamp(active_zone + randomValue(-1, 1), 1, 10);
    }
